 ...
 *
 */
class stateful_cas_castext2_statefulcs extends stack_cas_castext2_block {
    ...
    public function compile($format, $options): ?MP_Node {
        ...
        $r = new MP_List([new MP_String('%css')]);
    ...
    public function validate_extract_attributes(): array {
        ...
        foreach ($this->params as $key => $value) {
            $r[] = stack_ast_container_silent::make_from_teacher_source(
                $value, 'ct2:statefulcs', new stack_cas_security());
        }
        return $r;
    ...
    public function validate(&$errors=[], $options=[]): bool {
        if (!array_key_exists('key', $this->params)) {
            $errors[] = new $options['errclass']('The statefulcs block must always have a key for the string template.',
