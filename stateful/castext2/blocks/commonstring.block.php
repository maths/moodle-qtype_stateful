 ...
 *
 * This is the Stateful version the only difference is with the identifier
 * and with the source of the strings. Other than that everything is the same
 * this just does not come with STACK.
 */
class stateful_cas_castext2_commonstring extends stack_cas_castext2_block {
    ...
    public function compile($format, $options): ?string {
        ...
        // one internally "%css", note that the processors need to know of this.
        if (count($this->params) == 1) {
            return '["%css",' . stack_utils::php_string_to_maxima_string($this->params['key']) . ']';
        } else {
            $r = ['["%css"',stack_utils::php_string_to_maxima_string($this->params['key'])];
    ...
    public function postprocess(array $params, castext2_processor $processor): string {
        ...
        return stateful_string($params[1], $args);
    }
