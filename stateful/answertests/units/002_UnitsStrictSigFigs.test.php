...
class stack_test_UnitsStrictSigFigs implements stack_answertest {

    public function codename(): string {
        return 'UnitsStrictSigFigs';
    }
...
'UnitsStrictSigFigs requires options to tell how many significant figures are needed and possibly how many are acceptable.'
            ;
        } else {
        ...
        if ($sans === null || trim($sans) === '') {
            $err['sans'] = 'UnitsStrictSigFigs needs a students answer.';
        } else {
        ...
        if ($tans === null || trim($tans) === '') {
            $err['tans'] = 'UnitsStrictSigFigs needs a teachers answer.';
        } else {
...
'UnitsStrictSigFigs needs the students answer to be a raw input value for inspection of significant digits.'
            ;
        } else {
            $input = $input_definitions->get(trim($sans));
            if ($input === null || (!($input->get_type() === 'algebraic') && !(
                $input->get_type() === 'units'))) {
                $err['sans'] =

'UnitsStrictSigFigs answertest needs the students answer to come from an Algebraic or preferably from an Units type input.'
                ;
            }
        }
    ...
    public function cascall(
        ...
        return "ATUnitsStrictSigFigs($sans,$tans,ev($options,simp)" .
